...
namespace Engine
{
    ...
    void Shader::Compile(const std::string &_vertexShaderFilePath, const std::string &_fragmentShaderFilePath)
    {
        ...
        if (m_vertexShaderId == 0)
            FatalError("Vertex shader failed to be created!",5);

        //Getting fragment shaderID
        m_fragmentShaderId = glCreateShader(GL_FRAGMENT_SHADER);
        if (m_fragmentShaderId == 0)
            FatalError("Fragment shader failed to be created!",6);
    ...
    void Shader::Link()
    {
        ...
        if (isLinked == GL_FALSE)
        {
            ...
            FatalError("Shader failed to link!\nOpengl Error: " + std::string(infoLog[0], maxLength), 7);
        } else {
            m_isLinked = true;
        }
    ...
    GLint Shader::GetUniformLocation(const std::string &_uniformName)
    {
        ...
        if (location == GL_INVALID_INDEX)
            FatalError("Uniform " + _uniformName + " not found in shader!",8);
    ...
    void Shader::CompileShaderFile(const std::string &_filePath, unsigned int &_id)
    {
        ...
        if (shaderFile == nullptr)
            FatalError("Unable to open file \"" + _filePath + "\"",9);
